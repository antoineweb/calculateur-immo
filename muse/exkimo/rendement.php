...
<!-- /search -->
            ...
            <div id="faq-header" class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="breadcrumb-pim breadcrumb-pim-pub"><a href="https://www.rendement-immo.fr">Accueil</a> <i class="fal fa-angle-double-right"></i> <a href="https://www.rendement-immo.fr/calculs-automatiques/">Calculs automatiques</a> <i class="fal fa-angle-double-right"></i> Calcul de rendement immobilier <?php echo "sddsdd PHP "; ?></div>
                    </div>
                </div> 
                   ...
                   <form method="get" class="form" enctype="multipart/form-data" action="?bla=44&blala=55">
                            <div class="row calculspim">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        <div class="input-group mb-2 mr-sm-2">
                                            <input type="number" class="form-control form-rendement" name="revenus_locatifs" placeholder="Revenus locatifs après travaux"  <?php if (isset($_GET["revenus_locatifs"]) AND $_GET["revenus_locatifs"]!='' ) echo 'value="'.$_GET["revenus_locatifs"].'""'; ?> >
                                            <div class="input-group-prepend">
                                                  <div class="input-group-text">€</div> 
                                            </div>
...
if ($_GET["loyer"]<$_GET["revenus_locatifs"]) 
$loyer=$_GET["revenus_locatifs"];
else
$loyer=$_GET["loyer"];
...
<h2>Résultat</h2>
    <div><span class="label-calcul">Prix d’achat Initale : 
      <button href="#" data-toggle="tooltip" class="btn tooltip-pim" title="" data-original-title="Le prix de vente/achat convenu, hors « frais d’acte »."><i class="fas fa-info-circle" aria-hidden="true"></i></button></span> <span class="font-weight-bold">
        ...
        <?php echo $prix." €";    e?></span></div>
    <div><span class="label-calcul">Rendement très brut (%) : <button href="#" data-toggle="tooltip" class="btn tooltip-pim" title="" data-original-title="Résultat en % du calcul suivant: loyer annuel existant divisé par le prix achat hors frais d’acte."><i class="fas fa-info-circle" aria-hidden="true"></i></button></span> <span class="font-weight-bold"><?php echo round($rendement2,1); ?> %</span></div>
    ...
    <div><span class="label-calcul">Rendement brut (%) : <button href="#" data-toggle="tooltip" class="btn tooltip-pim" title="" data-original-title="Loyer annuel existant divisé par prix d’achat acte en main."><i class="fas fa-info-circle" aria-hidden="true"></i></button></span> <span class="font-weight-bold"><?php echo round($rendement3,1); ?> %</span></div>
    ...
    <div><span class="label-calcul">Rendement brut sur investissement total (%): <button href="#" data-toggle="tooltip" class="btn tooltip-pim" title="" data-original-title="Revenu locatif (le cas échéant, après travaux) divisé par investissement total."><i class="fas fa-info-circle" aria-hidden="true"></i></button></span> <span class="font-weight-bold"><?php echo round(100/(($prix+$frais_acte+$travaux)/($loyer*12*($occupation/100))),1); ?> %</span></div>
    ...
    <div><span class="label-calcul">Rendement net (%) : <button href="#" data-toggle="tooltip" class="btn tooltip-pim" title="" data-original-title="Revenu locatif réel divisé par investissement total."><i class="fas fa-info-circle" aria-hidden="true"></i></button></span> <span class="font-weight-bold">
        <?php echo round(100/(($prix+$frais_acte+$travaux)/(($loyer*12)-($precompte+$gestion+$assurances+$_GET["reparations"]+$IPP)*($occupation/100))),1);
